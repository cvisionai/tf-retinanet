...
class Generator(tf.keras.utils.Sequence):
    ...
    def __init__(
        ...
        tile_params={'x_stride':0,'y_stride':0,'x_strides':0,'y_strides':0},
    ...
    def __from_config__(
        ...
        Generator.__init__(
            ...
            image_crop_width        = config['image_crop_width'],
            image_crop_height       = config['image_crop_height'],
    ...
    def calc_box_overlap(self, ann, crop_params):
        """ Determine if a box is in an image crop.
        """
        crop = np.asarray([
            float(crop_params.get('x')),
            float(crop_params.get('y')),
            float(crop_params.get('x') + crop_params.get('w')),
            float(crop_params.get('y') + crop_params.get('h'))
        ])

        iw = np.minimum(crop[2], ann[2]) - \
             np.maximum(crop[0], ann[0]) + 1
        ih = np.minimum(crop[3], ann[3]) - \
             np.maximum(crop[1], ann[1]) + 1
        ba = (ann[2] - ann[0]) * \
             (ann[3] - ann[1])
    ...
    def crop_annotations(self, annotations, crop_params):
        """ Filter annotations based on image crop parameters.
        """
        filtered_boxes = []
        filtered_labels = []
        #overlaps = self.calc_box_overlap(annotations.get('bboxes'), crop_params)

        for box,label in zip(annotations.get('bboxes'),annotations.get('labels')):
        #for i,overlap in enumerate(overlaps):
            overlap = self.calc_box_overlap(box,crop_params)
            #print(overlap)
            if overlap > 0.5:
                #print(f"Original Box: {box}")
                box[0] = np.maximum(box[0] - crop_params.get('x'),0)
                box[2] = np.minimum(box[2] - crop_params.get('x'),crop_params.get('w'))
                box[1] = np.maximum(box[1] - crop_params.get('y'),0)
                box[3] = np.minimum(box[3] - crop_params.get('y'),crop_params.get('h'))
                #print(f"New Box: {box}")
                filtered_boxes.append(box)
                filtered_labels.append(label)

        filtered_annotations = {}
        filtered_annotations['bboxes'] = np.asarray(filtered_boxes)
        filtered_annotations['labels'] = np.asarray(filtered_labels)
    ...
    def crop_image(self, image):
        """ Generate a random crop of an image.
        """
        x_start = int(np.random.random() * (image.shape[1] - self.image_crop_width))
        y_start = int(np.random.random() * (image.shape[0] - self.image_crop_height))
        cropped_image = image[y_start:y_start+self.image_crop_height,
                                x_start:x_start+self.image_crop_width,:]
        
        return cropped_image, {'x' : x_start, 'y' :y_start, 'w' : self.image_crop_width, 'h' : self.image_crop_height}

    def crop_val_image(self, image, crop_params):
        """ Generate a crop of an image.
        """
        x_start = crop_params.get('x')
        y_start = crop_params.get('y')
        cropped_image = image[y_start:y_start+self.image_crop_height,
                                x_start:x_start+self.image_crop_width,:]
    ...
    def preprocess_group_entry(self, image, annotations):
        ...
        if self.image_crop_width is not None:
            image, crop_params = self.crop_image(image)
            annotations = self.crop_annotations(annotations, crop_params)
